...
Releases are named for the full patch, so `8.4.24` and `8.4.25` are separate assets and lerd pins one of them per minor. `lerd php:update` re-reads the pins, downloads anything newer and restarts the pools running it. Each release also carries `native-php.yaml`, the pin block generated by the builds themselves, which is what lerd's tools manifest is updated from.
